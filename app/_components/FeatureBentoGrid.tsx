...
const SkeletonTwo = () => {
    ...
    React.useEffect(() => {
        // Generate random widths only on client side to avoid hydration mismatch
        const widths = arr.map(() => Math.random() * (100 - 40) + 40 + "%");
        ...
        >
            {arr.map((_, i) => (
                <motion.div
                    ...
                    variants={variants}
                    style={{
                        maxWidth: randomWidths[i] || "50%", // fallback to 50% during hydration
                    }}
